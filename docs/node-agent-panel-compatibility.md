...
遥测使用与当前 Go agent 相同的 ACP 字段和 3 秒采样周期。进程实例 ID、递增采样序号和单调时间跨连接保留；每次遥测重连单独建立连接，并通过 `x-acp-telemetry-ready` 响应头重新建立时间基准。建连和握手分别有 10 秒预算，单条发送等待最多 1 秒，不给整条长连接设置固定生命周期。
